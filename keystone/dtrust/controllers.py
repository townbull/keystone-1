...
class DTrustV3(controller.V3Controller):
    collection_name = "dtrusts"
    member_name = "dtrust"
    ...
    def base_url(cls, path=None):
        ...
        url = endpoint + 'v3/OS-DOMAIN-TRUST'
    ...
    def get_dtrust(self, context, trust_id):
        ...
        try:
            ...
            if trust.get('expires_at') is not None:
                ...
                trust['expires_at'] = (timeutils.parse_isotime
                                       (trust['expires_at']))
                new_dtrust = self.dtrust_api.create_dtrust(
                dtrust_id=uuid.uuid4().hex,
                dtrust=dtrust,
                roles=clean_roles)
            self._fill_in_roles(context,
                                new_dtrust,
                                global_roles)
            return DomainTrustV3.wrap_member(context, new_dtrust)
